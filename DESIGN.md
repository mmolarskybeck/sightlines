...
- Use the shared shadcn-style Tooltip for icon-only controls and contextual explanations. Tooltip chrome is ink with white text; rich placement tooltips keep the same chrome and may add structured content. Never rely on a native `title` as the only explanation.
