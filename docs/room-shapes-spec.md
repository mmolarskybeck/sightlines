...
- **No numeric length editing of arbitrary polygon walls.** "Set this wall to 5 m" on an irregular polygon is underdetermined (which neighbor gives?). Numeric width/depth stays rectangle-gated exactly as today; a future "edit length, choose the anchor" tool can lift this.
