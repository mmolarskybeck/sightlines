...
```
No canvas access inside either. Operates in wall-local real-world units (x = distance from wall start, y = height from floor — center-anchored, not top-left). Pixel-to-world threshold conversion happens at the call site using current zoom. **Per-axis resolution:** x and y resolve independently. Each axis pools targets with that axis or "both", filters by threshold, sorts by priority → distance → id, and applies its own winner. This allows, for example, an artwork to ride the eyeline (y centerline snap) while simultaneously snapping its x to the grid. Explicit priority order when multiple targets are in range: floor (doors only, rank 0) > shelf-top (rank 0.5) > centerline (rank 1) > floor (other kinds, rank 1.5) > neighbor-center (rank 2) > neighbor-edge (rank 3) > grid (rank 4) with a stable tiebreak. `shelf-top` is offered only for a work whose x-span already overlaps a shelf on the same wall, and it outranks the centerline because a work that is standing on something is standing on it (USER DECISION 2026-09-18). Grid targets come from the shared precision system (§5.5), not from ad hoc renderer math. Hysteresis on break-free so it feels magnetic, not jittery — since hysteresis is inherently stateful (harder to break free of a snap than to enter one), the *caller* tracks the previous targets per axis and passes them in as `previousSnapTargetIds: { x?, y? }`; `resolveSnap` stays pure and testable rather than quietly accumulating state that would otherwise leak canvas/view concerns back into the snapping layer.
...
| Sync | Dropbox API, OAuth 2.0 + PKCE, no backend | See §6 — shipped: versioned backups, one-way share links, cloud project browser, and canonical cross-device sync. The only server code is the Cloudflare worker's stateless share-link relay plus the same-origin analytics endpoint; the relay serves share-link downloads. |
...
// Also in the union: `kind: "wall-text"` didactic panels and `kind: "case"`
// wall-mounted vitrines (both schema v4, 2026-07-18), plus `kind: "shelf"`
// (schema v7, 2026-09-18) — a slab works stand on, with yMm the slab CENTRE,
// heightMm its thickness and depthMm its protrusion from the wall face. Their
// construction lives in pure glyph modules (`caseGlyphs.ts`, `doorGlyphs.ts`,
// `monitorGlyphs.ts`, `shelfGlyphs.ts`) so canvas, PDF and the export preview
// cannot drift.
...
type OpeningWallObject = WallObjectBase & {
  ...
  leaf?: DoorLeaf            // doors only; absent = a plain doorway
}

type DoorLeaf = {
  hingeAtStart: boolean // hinge at the jamb nearer the wall's AUTHORED start
  swingsToLeft: boolean // opens toward the LEFT of authored start -> end
}
```

**Placement dimensions are the behavioral image footprint; display overrides are metadata.** `displayDimensionsOverride` may preserve a framed, matted, or placeholder provenance/display value without editing the shared library record, but geometry never resolves it. Every persisted or imported wall placement uses its stored `widthMm`/`heightMm` for image footprint; a future writer must explicitly rebake those stored dimensions (including partial/placeholder cases) when that is intended. Wall dimension edits resync matching wall placements without an explicit override; override-bearing placements retain their explicitly stored footprint until deliberately rebaked. Mat/frame-only edits never touch placement geometry. Floor artwork has a separate physical support: its stored box width/depth/height, angle, and optional `baseHeightMm` describe the object on the floor, while the artwork record's dimensions describe the work rendered on selected faces. At placement the support is seeded from the work's effective size; afterward the two may diverge, and selected face images are centered at the work's own aspect without distortion. Tooltips may continue to show display/provenance overrides, but they do not alter geometry.

**A shelf is its own wall object, and what it carries is never stored.** Several works share one slab, so the slab exists independently of any of them (USER DECISION). Its *riders* — the works standing on it — are derived from geometry at move time on every path (elevation drag and nudge, plan drag and nudge, inspector x/top edits): same wall, overlapping x-spans, bottom edge within `SHELF_TOP_SNAP_TOLERANCE_MM` of the slab's top face. `shelfRiders.ts` is the entire definition; `WallObjectBase.groupId` remains dead data and must not be pressed into service as a membership list. In plan a shelf reanchors across walls like artwork but rigidly — one common delta for the whole assembly, refused when the union does not fit — and deleting a shelf leaves its works exactly where they are, saying how many.

A door or window on a boundary shared by two rooms is **one physical opening with two stored faces**, not a pairing the curator manages. Wall topology is derived independently of any opening; on explicitly opted-in, transaction-scoped geometry edits, deterministic reconciliation adopts an aligned exposed counterpart, creates a missing face on an empty shared wall, or realigns an existing pair in the same undo step. Direct opening edits classify the pair before mutation and are refused atomically when both faces cannot remain synchronized. Load repair may link or realign existing faces but never invents a missing one; ambiguous boundaries, occupied mirror slots, legacy one-sided openings, overhangs, and lost boundaries persist as named issues with guarded inspector resolutions. Pair identity survives rooms moving apart until the curator chooses to keep both doors separately or keep only the selected one. `connectsToObjectId` remains the reciprocal storage link because one wall can carry several openings, but healthy pair identity is topology-derived: the inspector shows only `Connects Room A ↔ Room B`, with no connection dropdown or Disconnect action. A healthy aligned pair gives the 3D renderer one true shared clear opening; unresolved or exposed openings remain capped. The `Floor`/`RoomPlacement` structure is the shared coordinate spine.

**A door is a doorway unless the curator says otherwise.** `kind: "door"` remains a floor-to-head void; `leaf` is what adds a hinged panel, and it is absent by default, so every project written before it existed is a project of plain doorways. Both `DoorLeaf` flags are stated against the wall's **authored** `start -> end` direction, never against "the room's interior": interior-ness is only the left normal for counter-clockwise rooms, and clockwise rooms remain supported (`deriveRoom` reverses their 3D panels). Room winding is therefore consulted exactly once — when choosing the default handing at the moment a curator makes a door hinged — and never at render time. This keeps every renderer a pure function of the wall it draws, at the cost that reversing a wall's authored direction would mirror the handing, the same exposure the wall-local `xMm` has always had.

Handing is a property of the physical door, so on a real shared boundary it mirrors to both faces (`hingeAtStart` and `swingsToLeft` both invert — twin walls run opposite directions *and* have opposite interiors). The gate is `areSharedBoundaryWalls`, exactly as position mirroring already uses: a legitimate legacy pair on unrelated, non-facing walls keeps independent leaves, because mirroring across walls that never faced each other is meaningless. Pair formation resolves disagreement by a stated rule rather than by whichever half sorts first — **hinged wins** over a plain doorway, so adopting a pair can never silently erase the only leaf in it; two conflicting hinged halves defer to the one the curator acted on. Handing never becomes a shared-opening *issue*: there is no conflict reason for it and nothing for the curator to resolve.

The leaf is drawn shut everywhere and asserts only what the model knows. Plan shows the swing arc, and it is the only glyph in the app that paints outside its own rect — it is decoration over the thin opening rectangle, never part of it, so hit-testing, marquee selection and `renderedRect` are untouched and the arc is pointer-transparent. Elevation shows a leaf panel and a knob but **no** swing indication, because an elevation cannot honestly show depth perpendicular to the wall. 3D draws one thin leaf per physical door (deduplicated across an aligned pair, which would otherwise z-fight) and suppresses the opening cap that would fight it. Swing clearance is advisory in v1: nothing about the arc blocks or warns on placement.

**A full-span perimeter wall may be explicitly open.** An open wall has no placeable surface: plan and 3D show the gap, artwork and opening tools are disabled for that wall, and PDF elevation omits the wall page while retaining the wall in navigation/list context. The action is all-or-nothing for the wall span; partial-span openings remain ordinary wall objects so a room cannot silently become a half-open surface. Open walls are excluded from shared-opening reconciliation and partition clearance measurements because there is no physical wall face to reconcile against or measure to.
...
**1C — Professional layout behaviors.** Doors/windows/blocked zones + `validatePlacement` collision (§2) · multi-select, grouping, group drag, group-centerline snap · equal-distribution spacing · floor objects (plan view only) · simple derived 3D preview, orbit camera · checklist panel: thumbnail, core fields, sort, artist grouping, and temporary search. **Shipped.**
...
- **Shipped:** topology-derived shared doors/windows with reciprocal stored faces, scoped atomic reconciliation, persistent issue resolution, one-step undo, quiet inspector status, and honest 3D see-through/capped treatment.
- **Shipped:** optional hinged door leaves with authored-direction handing, shared-boundary mirroring, plan swing arcs, elevation leaves, and deduplicated 3D panels.
- **Shipped:** explicit full-span open-wall state, with placement guards, open plan/3D gaps, and PDF elevation omission.
- **Shipped:** partition elevation profiles and PDF projection, plus the floor-object presentation refinements described in §4.2.
