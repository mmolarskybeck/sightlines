...
**1A — Geometry spine.** Units parser/formatter (§5) · versioned project schema + Zod validation (§2) · repository interface (§2) · single-room footprint editing with vertex-ID-based wall identity (§4.2) · wall elevation view (empty, no artwork yet) · local save/load, JSON export/import. No images, no 3D yet — this phase is about the domain model and geometry transforms being correct and boring.
