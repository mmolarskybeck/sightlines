...
* Use `npm run test:e2e` for committed browser-level verification. Keep Playwright tests under `e2e/`; use `.claude/skills/verify/driver.mjs` only for ad-hoc screenshots, evaluation, and interactive debugging when that local skill is available.
