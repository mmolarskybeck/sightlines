...
* Use `npm run test:e2e` for committed browser-level verification. Browser-dependent interaction regressions (layout shift, Radix portal/attribute interplay, pointer/gesture ordering, focus/blur side effects) get a Playwright regression spec in `e2e/` when practical. Headless (`npm run test:e2e`) for CI; `npm run test:e2e:ui` headed mode for local debugging. Specs import `e2e/fixtures.ts`; jsdom/Vitest remains for domain logic, hook state machines, and DOM structure. Use `.claude/skills/verify/driver.mjs` only for ad-hoc screenshots, evaluation, and interactive debugging when that local skill is available.
